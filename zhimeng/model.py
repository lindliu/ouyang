...
#     print(classification_report(y_test, model_SVC.predict(X_test)))
    F1_SVC = f1_score(y_test, model_SVC.predict(X_test), average='weighted')
    ### explain model
...
#     print(classification_report(y_test, model_XGBoost.predict(X_test)))
    F1_XGBoost = f1_score(y_test, model_XGBoost.predict(X_test), average='weighted')
    ### explain model
...
#     print(classification_report(y_test, model_RF.predict(X_test)))
    F1_RF = f1_score(y_test, model_RF.predict(X_test), average='weighted')
    ### explain model
    ...
    print('NB, SVC, LR, XGBoost, RF: ', np.array([F1_NB, F1_SVC, F1_LR, F1_XGBoost, F1_RF]))
...
#         np.c_[importance_LR, importance_XGBoost, importance_RF], \
#                 np.array([F1_LR, F1_XGBoost, F1_RF])
